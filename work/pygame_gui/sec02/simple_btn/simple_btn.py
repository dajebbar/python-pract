...
class SimpleButton:
    ...
    def handle_event(self, event_obj):
        ...
        event_point_in_btn_rect = self.rect.collidepoint(event_obj.pos)

        if self.state == SimpleButton.STATE_IDLE:
            if (event_obj.type == MOUSEBUTTONDOWN) and event_point_in_btn_rect:
                self.state = SimpleButton.STATE_ARMED
        
        elif self.state == SimpleButton.STATE_ARMED:
            if (event_obj.type == MOUSEBUTTONUP) and event_point_in_btn_rect:
                self.state = SimpleButton.STATE_IDLE
                return True  # clicked!
            
            if (event_obj.type == MOUSEMOTION) and (not event_point_in_btn_rect):
                self.state = SimpleButton.STATE_DISARMED
        
        elif self.state == SimpleButton.STATE_DISARMED:
            if event_point_in_btn_rect:
                self.state = SimpleButton.STATE_ARMED
